...
# eaton.years |> count()
# rep(0, eaton.years |> count())
# eaton.which.fire
# palisades.which.fire

# (frap.la.intersect |> as.data.frame() |> filter(which.fire == 'Palisades') |> pull(block.area) |> unique())
# which.fire <- c(rep('Eaton', eaton.years |> count()), rep('Palisades', palisades.years |> count()))
...
# format(as.Date("2025-01-08", format="%Y-%m-%d"), "%Y")

# as.Date("2025-01-08", format="%Y-%m-%d")
...
#Pre-2025 Burned Area Proportion (%)
#Urban Morphology
# glimpse(combined.block.sf)
#Try a correlation lot
...
#Create an Eaton Parcel-Scael Data frame, correlation matrix and melted correlation matrix
...
fig3a <- ggplot(data = combined.melt |> filter(Var2 %in% c('Tpct.FA_65.and.over_yrs._C', 'zone_two_overlap_mean', 'zone_one_overlap_mean', 'zone_zero_overlap_mean', 
...
#Parcel melt
fig3b <- ggplot(data = combined.parcel.melt |> filter(Var2 != 'pop.total'), aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
  scale_fill_gradient2(midpoint = 0, mid ="grey70", 
                       limits = c(-0.5, +0.5)) +
  labs(title = "Parcel", 
...
fig3 <- ggarrange(fig3a, fig3b, nrow = 1, ncol = 2, common.legend = TRUE, legend = 'right',  widths = c(1.0, 0.50))
fig3

ggsave('Fig29_combined_neighborhood_correlation_plot.png',
       plot = fig3,
       path = 'C://Users//CarlNorlen//mystuff//la-urban-fires//figures',
       scale = 1,
       width = 20,
...
p12a <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), mapping = aes(color = which.fire, x = structure.basal.area, y = destroy_pct)) +
...
p12b <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), mapping = aes(color = which.fire, x = tree.cover.2022, y = destroy_pct)) +
  geom_point(size = 1, alpha = 0.5) +
  stat_cor(mapping = aes(label = paste(after_stat(rr.label), after_stat(p.label), sep = "~`,`~"), 
           color = which.fire), label.x.npc = 0.05, label.y.npc = 0.99, p.accuracy = 0.01, size = 5) +
  geom_smooth(method = 'lm', mapping = aes(linetype = which.fire, color = which.fire), linewidth = 1.5) +
...
p12c <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), 
...
p12d <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & !is.na(ww_bbg_pct_bachelor_C) & UR20 == 'U'), 
...
p12e <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & !is.na(PercapitaInc) & UR20 == 'U'), 
...
p12f <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & !is.na(PercapitaInc) & UR20 == 'U'), 
...
f12 <- ggarrange(p12a, p12b, p12c, p12d, p12e, p12f, nrow = 2, ncol = 3, align = "hv", common.legend = FALSE, labels = c('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i'))
f12

#Save the figure
ggsave('Fig12_fire_damage_correlation_grid.png',
       plot = f12,
       path = 'C://Users//CarlNorlen//mystuff//la-urban-fires//figures',
       scale = 1,
       width = 36,
...
p18a <- ggplot(data = all.join |> filter(DAMAGE_1 != "Inaccessible" & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
  ...
  # geom_point(size = 1, alpha = 0.5) +
  # geom_density(mapping = aes(x = as.numeric(YearBuilt1))) +
  stat_cor(mapping = aes(label = paste(after_stat(rr.label), after_stat(p.label), sep = "~`,`~"), 
...
#Zone Zero Overlaps
p18b <- ggplot(data = all.join |> filter(DAMAGE_1 != 'Inaccessible' & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
                                           !is.na(YearBuilt1) & YearBuilt1 > 1500), 
...
p18c <- ggplot(data = all.join |> filter(DAMAGE_1 != 'Inaccessible' & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
                                           !is.na(YearBuilt1) & YearBuilt1 > 1500), 
...
p18d <- ggplot(data = all.join |> filter(DAMAGE_1 != 'Inaccessible' & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
                                           !is.na(YearBuilt1) & YearBuilt1 > 1500), 
...
p18e <- ggplot(data = all.join |> filter(DAMAGE_1 != 'Inaccessible' & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
                                           !is.na(YearBuilt1) & YearBuilt1 > 1500), 
...
p18f <- ggplot(data = all.join |> filter(DAMAGE_1 != 'Inaccessible' & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
                                           !is.na(YearBuilt1) & YearBuilt1 > 1500), 
...
#Pre-fire Structure basal area
p18g <- ggplot(data = all.join |> filter(DAMAGE_1 != 'Inaccessible' & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
                                           !is.na(YearBuilt1) & YearBuilt1 > 1500), 
               mapping = aes(x = as.numeric(structure.basal.area), y = damage.binary)) +
  geom_bin2d() +
  # ggtitle('Parcel-Scale') +
  scale_fill_gradient2(limits = c(0,2300), breaks = c(500, 1000, 1500, 2000), midpoint = 1150, low = "cornflowerblue", mid = "yellow", high = "red",
                       na.value = 'transparent', name = "Count") +
  stat_cor(mapping = aes(label = paste(after_stat(rr.label), after_stat(p.label), sep = "~`,`~"), 
...
#Pre-fire urban tree cover
p18h <- ggplot(data = all.join |> filter(DAMAGE_1 != 'Inaccessible' & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
                                           !is.na(YearBuilt1) & YearBuilt1 > 1500), 
...
#Total occupants
p18i <- ggplot(data = all.join |> filter(DAMAGE_1 != 'Inaccessible' & STRUCTURECATEGORY %in% c('Single Residence', 'Multiple Residence' , 'Mixed Commercial/Residential') &
                                           !is.na(YearBuilt1) & YearBuilt1 > 1500), 
...
f18 <- ggarrange(p18a, p18b, p18c, p18d, p18e, p18f, p18g, p18h, p18i, nrow = 3, ncol = 3, align = "hv", common.legend = FALSE, labels = c('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i'))
f18
...
#Supplemental Urban Morphology
#Mean number of structure overlaps in Zones 0 & 1
#Pre-fire median year structure built
p24a <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), mapping = aes(color = which.fire, x = fire.area.1910to2023.pct, y = destroy_pct)) +
  geom_point(size = 1, alpha = 0.5) +
  stat_cor(mapping = aes(label = paste(after_stat(rr.label), after_stat(p.label), sep = "~`,`~"), 
...
p24b <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), mapping = aes(color = which.fire, x = year.built.median, y = destroy_pct)) +
  geom_point(size = 1, alpha = 0.5) +
  stat_cor(mapping = aes(label = paste(after_stat(rr.label), after_stat(p.label), sep = "~`,`~"), 
                         color = which.fire), label.x.npc = 0.05, label.y.npc = 0.99, p.accuracy = 0.01) +
  geom_smooth(method = 'lm', mapping = aes(linetype = which.fire, color = which.fire)) +
...
#Median structure replacement value
p24c <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), 
...
p24d <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), 
...
p24e <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), mapping = aes(color = which.fire, x = zone_one_overlap_mean, y = destroy_pct)) +
  geom_point(size = 1, alpha = 0.5) +
  stat_cor(mapping = aes(label = paste(after_stat(rr.label), after_stat(p.label), sep = "~`,`~"), 
                         color = which.fire), label.x.npc = 0.05, label.y.npc = 0.99, p.accuracy = 0.01) +
  geom_smooth(method = 'lm', mapping = aes(linetype = which.fire, color = which.fire)) +
...
p24f <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), 
...
f24 <- ggarrange(p24a, p24b, p24c, p24d, p24e, p24f, nrow = 2, ncol = 3, align = "hv", common.legend = FALSE, labels = c('a', 'b', 'c', 'd', 'e', 'f'))
f24

#Save the figure
ggsave('Fig24_fire_damage_urban_morphology_correlation_grid.png',
       plot = f24,
       path = 'C://Users//CarlNorlen//mystuff//la-urban-fires//figures',
       scale = 1,
       width = 36,
...
#Mean number of structure overlaps in Zones 0 & 1
#Pre-fire median year structure built
p25a <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & !is.na(PercapitaInc) & UR20 == 'U'), 
...
# summary(combined.block.sf |> as.data.frame())
p25b <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & !is.na(PercapitaInc) & UR20 == 'U'), 
...
#Median structure replacement value
p25c <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & !is.na(PercapitaInc) & UR20 == 'U'), 
...
p25d <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & !is.na(PercapitaInc) & UR20 == 'U'), 
...
p25e <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & !is.na(PercapitaInc) & UR20 == 'U'), 
...
#Mean number of structure overlaps in Zones 0, 1, & 2
p25f <- ggplot(data = combined.block.sf |> as.data.frame() |>  filter(as.numeric(fire.area.2025.pct) > 0 & !is.na(structure.count) & !is.na(structure_value_median) & UR20 == 'U'), 
...
f25 <- ggarrange(p25a, p25b, p25c, p25d, p25e, p25f, nrow = 2, ncol = 3, align = "hv", common.legend = FALSE, labels = c('a', 'b', 'c', 'd', 'e', 'f'))
f25

#Save the figure
ggsave('Fig25_fire_damage_socioeconomic_correlation_grid.png',
       plot = f25,
       path = 'C://Users//CarlNorlen//mystuff//la-urban-fires//figures',
       scale = 1,
       width = 36,
...
f20 <- ggarrange(p20a, p20b, nrow = 2, ncol = 1, common.legend = FALSE, labels = c('a', 'b'))
f20

#Save the figure
ggsave('Fig20_map_after_2008_pct.png',
       plot = f20,
       path = 'C://Users//CarlNorlen//mystuff//la-urban-fires//figures',
       scale = 1,
       width = 16,
